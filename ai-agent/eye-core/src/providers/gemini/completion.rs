...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "eye",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "eye",
...
pub mod gemini_api_types {
    ...
    /// ### eye Note:
    /// Can be used to cosntruct a typesafe `additional_params` in eye::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
